...
export class AppService implements OnModuleInit {
  ...
  async createQuiz(content: string): Promise<Quiz[]> {
    ...
    const prompt = `Create a diverse mock test based on the following text. Formulate 10 unique and thought-provoking questions that assess critical thinking and comprehension skills without direct repetition. Ensure that the questions do not require knowledge beyond the provided text. For each question, provide 5 possible answer choices and clearly designate the correct answer. Additionally, include an answer key that matches each question with its correct answer.
    ...
    const stringTest = await this.openai.chat.completions.create({
      messages: [
        {
          role: 'system',
          content: `You are a test maker who makes accurate tests in the format: \n\n{question}?\nA) {choice}\nB) {choice}\nC) {choice}\nD) {choice}\nE) {choice}\n\n{question2}?\nA) {choice}\nB) {choice}\nC) {choice}\nD) {choice}\nE) {choice}\n\nAnd so on for as many questions as needed.\n\nAnswer Key: {Provide the correct answer choices in a single line WITHOUT question numbers and WITH the word 'Answer Key:' at the beginning, e.g., 'Answer Key: ABEDC...'}`,
        },
        { role: 'user', content: prompt },
      ],
      model: 'gpt-3.5-turbo',
    });
    const res: Quiz[] = parseResponse(
      stringTest['choices'][0]['message']['content']
    );
