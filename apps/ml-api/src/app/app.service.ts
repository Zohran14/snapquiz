...
export class AppService implements OnModuleInit {
  ...
  async ocr(input: Buffer): Promise<OcrResult> {
    const [result] = await this.imageClient.documentTextDetection(input);
    const fullTextAnnotation = result.fullTextAnnotation;

    const validate = await this.openai.chat.completions.create({
      messages: [{ role: 'user', content: `Analyze the text below and determine if it is a valid document. Do not judge based on spelling or grammar. If so, respond by cleaning up any spelling mistakes in the content below. WRITE NOTHING ELSE. If the text below is not a valid document respond with ‘no’:\n\n${fullTextAnnotation.text}` }],
      model: 'gpt-3.5-turbo',
    });
